...
def solve_mkdv(J=4, alpha=-1, beta=1e-2, mu=40, x0=1/4, widthTol=0.05, scaling=0.35):
    # tf
    tf = (1 - 2 * x0) / (mu**2 * beta)
    # dt
    ...
    ## scaling
    scale_weights = lambda w: sw(w, weightPow=scaling)
    ...
    # plt.plot(X.flatten(), u0x.flatten(), '-o'),plt.show()
    ...
    while solver.t < tf and solver.successful:
        ...
        reprint('t=%g'%solver.t)# + "%g vs %g"%(curmid, newmid))
        if np.abs(curmid - newmid) > widthTol:
            ...
            curmid = newmid
            # ax = plot2D(Xo, solver.y, bShow=False, label='old')
            # plot2D(X, ucur, label='new', ax=ax)
    XX = np.array(xres)
...
if __name__ == "__main__":
    X, T, U, Ue, tf = solve_mkdv(J=5, x0=3/10., widthTol=0.05, mu=40, scaling=0.)
